...
function AdminApp() {
  ...
  const updateStatus = (id: string, status: Status) => {
    ...
    writeStorage(REQUEST_KEY, updated);
    setRequests(updated);
  ...
  const deleteQuote = (id: string) => {
    ...
    writeStorage(QUOTE_KEY, updated);
    setQuotes(updated);
    setSelected(null);
  };
  ...
  const deleteMessage = (id: string) => {
    ...
    writeStorage(MESSAGE_KEY, updated);
    setMessages(updated);
  };
  ...
  const openWhatsApp = (phone: string, name: string, service: string, id: string) => {
    const msg = `Hello ${name}, this is Copper & Crown regarding your request ${id} for ${service}.`;
        ...
        <div className="admin-header-inner">
          <div className="admin-brand">
            ...
            <div>
              ...
              <small>ADMIN DASHBOARD · DEMO</small>
            </div>
          </div>
      ...
      <div className="admin-body">
        ...
        <div className="admin-toolbar">
          <div className="admin-search">
            <Search size={16} />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, ID, or service..." />
          </div>
          {tab === 'service' && (
        ...
        <div className="admin-demo-note">
          This is a demo dashboard using browser storage. A secure backend with authentication is required for production use.
        </div>

        {tab === 'service' && (
          <div className="admin-table-wrap">
            <table className="admin-table">
              <thead><tr><th>ID</th><th>Name</th><th>Service</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead>
              <tbody>
                {filteredRequests.length === 0 ? (
                  <tr><td colSpan={6} className="empty-row">No service requests found.</td></tr>
        ...
        {tab === 'quote' && (
          <div className="admin-table-wrap">
            <table className="admin-table">
              <thead><tr><th>ID</th><th>Name</th><th>Service</th><th>Contact Method</th><th>Date</th><th>Actions</th></tr></thead>
              <tbody>
                {filteredQuotes.length === 0 ? (
                  <tr><td colSpan={6} className="empty-row">No quote requests found.</td></tr>
                ) : filteredQuotes.map((q) => (
                  <tr key={q.id}>
                    <td className="mono">{q.id}</td>
                    <td>{q.name}</td>
                    <td>{q.service || '—'}</td>
                    <td>{q.contactMethod}</td>
            ...
            <table className="admin-table">
              ...
              <tbody>
                {filteredMessages.length === 0 ? (
                  <tr><td colSpan={6} className="empty-row">No messages found.</td></tr>
                  ...
                  <tr key={m.id}>
                    ...
                    <td className="action-cell">
                      <button className="row-action danger" onClick={() => deleteMessage(m.id)} title="Delete"><Trash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="admin-modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) setSelected(null); }}>
          <div className="admin-modal">
            ...
            <div className="modal-actions">
              <button className="button button-outline" onClick={() => copyText(selected.phone, selected.id)}>
                <Copy size={15} /> {copied === selected.id ? 'Copied!' : 'Copy phone'}
              </button>
              <button className="button button-outline" onClick={() => window.open(`tel:${selected.phone}`)}>
                <Phone size={15} /> Call customer
              </button>
              <a className="button button-whatsapp" href={whatsappUrl(`Hello ${selected.name}, this is Copper & Crown regarding your request ${selected.id}.`)} target="_blank" rel="noreferrer">
